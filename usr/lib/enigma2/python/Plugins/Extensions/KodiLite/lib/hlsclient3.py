...
#            print("In player_pipe block =", block)
            if block is None:
                return
    ...
    def play(self, header):
        ...
        """
        videopipe = open(STREAM_PFILE, "r+b", buffering=0)
        ...
        if len(variants) == 1:
            ...
            """
            print("Here in hlsclient.py autoChoice =", autoChoice)
            choice = max(autoChoice.items(), key=operator.itemgetter(1))[0]
            ...
            print("Here in hlsclient.py self.url =", self.url)
            # queue = Queue.Queue(1024)         # 1024 blocks of 4K each ~ 4MB buffer
        queue3 = queue.Queue(1024)
        self.thread = threading.Thread(target=self.player_pipe, args=(queue3, videopipe))
        self.thread.start()
        last_seq = -1
        targetduration = 5
        changed = 0
...
#                           print("Here in [hlsclient::play] chunk = ", chunk)
                        queue3.put(chunk, block=True)
                    last_seq = seq
                    changed = 1
            self._sleeping = True
            if changed == 1:
