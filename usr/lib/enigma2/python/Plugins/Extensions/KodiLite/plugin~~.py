...
from Components.config import getConfigListEntry, ConfigText, ConfigDirectory
...
# from Plugins.Extensions.KodiLite import adnutils
...
class XbmcPluginPics(Screen):
        ...
        # def openTest(self):
            ...
            # elif self.ipage == self.npage:
                ...
                # print("self.ipage , self.minentry, self.maxentry B=", self.ipage, self.minentry, self.maxentry)
                # i1 = 0
                # blpic = res_plugin_path + "Blank.png"
                # while i1 < 12:
                      # self["label" + str(i1+1)].setText(" ")
                      # self["pixmap" + str(i1+1)].instance.setPixmapFromFile(blpic)
                      # i1 = i1+1
            ...
            # while i < ln:
                ...
                # elif not os.path.exists(pic):
                    # if Utils.isFHD():
                        # picno = res_plugin_path + "defaultL.png"
                    # else:
                        # picno = res_plugin_path + "default.png"
...
class DelAdd(Screen):
    ...
    def okClicked(self):
        ...
        if sel is None:
            ...
            self.session.open(DelAdd2, 'scripts')
        elif sel == 2:
            self.session.open(DelAdd3, 'repos')
...
class DelAdd1(Screen):
    ...
    def __init__(self, session):
        Screen.__init__(self, session, type)
        self.skinName = "XbmcPluginScreenF"
        self.session = session
        self.list = []
        self["menu"] = List(self.list)
    ...
    def openTest(self):
        ...
        if Utils.isFHD():
            pic = res_plugin_path + "defaultL.png"
        self["pixmap1"].instance.setPixmapFromFile(pic)
        adds = "/usr/lib/enigma2/python/Plugins/Extensions/KodiLite/%s" % self.type
    ...
    def okClicked(self):
        ...
        cmd = "rm -rf '/usr/lib/enigma2/python/Plugins/Extensions/KodiLite/%s/" + plug + "'" % self.type
...
def main(session, **kwargs):
    ...
    if not os.path.exists("/etc/KodiLite/favorites.xml"):
        ...
        system(cmd)
    system("mkdir -p " + cfg.cachefold.value + "/xbmc")
    system("mkdir -p " + cfg.cachefold.value + "/xbmc/vid")
    system("mkdir -p " + cfg.cachefold.value + "/xbmc/pic")
    system("mkdir -p " + cfg.cachefold.value + "/xbmc/tmp")
    system("mkdir -p " + cfg.cachefold.value + "/xbmc/home/")
    system("mkdir -p " + cfg.cachefold.value + "/xbmc/database/")
    system("mkdir -p " + cfg.cachefold.value + "/xbmc/userdata/")
    system("mkdir -p " + cfg.cachefold.value + "/xbmc/userdata/Database/")
    system("mkdir -p " + cfg.cachefold.value + "/xbmc/home/addons/")
    system("mkdir -p " + cfg.cachefold.value + "/xbmc/home/addons/packages")
    print("In def main 4")
